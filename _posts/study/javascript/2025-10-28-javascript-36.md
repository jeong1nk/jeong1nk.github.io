...
- **🏷️디스트럭처링 할당(구조 분해 할당): 구조화된 배열과 같은 이터러블 또는 객체를 비구조화(destructuring, 구조 파괴)하여 1개 이상의 변수에 개별적으로 할당하는 것**
...
- **🏷️배열 디스트럭처링 할당은 배열과 같은 이터러블에서 필요한 요소만 추출하여 변수에 할당하고 싶을 때 유용하다.**
...
- 따라서 객체의 프로퍼티 키와 다른 변수 이름으로 프로퍼티 값을 할당받으려면 다름과 같이 변수를 선언한다.
...
- 객체 디스트럭처렁 할당을위한 변수에 기본값을 설정할 수 있다.
...
- 객체 디스트럭처링 할당은 객체를 인수로 전달받는 함수의 매개변수도 사용할 수 있다.
